...
| [4. Security](roles/4_security/tasks/main.yml) | UFW, SSH hardening, Fail2ban, unattended upgrades, preflight lockout checks, final validation | Useful host hardening; SSH transition and rollback are not yet safe enough |
...
| P0-02 | Identity | [The privileges task](roles/3_identity_access/tasks/05_privileges.yml) reconciles supplementary groups with `append: false`. This can remove memberships owned by packages, applications, or administrators outside Foundry. | Either declare that Foundry owns the complete supplementary-group set or preserve non-Foundry memberships and reconcile only Foundry-managed groups. Add a drift test proving that an unrelated group is preserved. |
| P0-03 | Security / inventory | The SSH transition derives the active port from `ansible_port`, but the inventory currently does not persist the new management port. A port change therefore leaves the old SSH port open/listening until inventory is changed manually. | Model the current and desired management endpoint explicitly. Use a two-phase migration: open/listen on the new port, validate a new controller connection, update inventory/connection state, then retire the old port. Test both `22 -> custom` and `custom -> custom` migrations. |
| P0-04 | Security | The SSH drop-in is placed before the complete effective configuration is validated. If merged `sshd -t` validation fails, an invalid file can remain on disk and break a later restart or reboot. | Stage the candidate, validate the complete merged configuration, atomically promote it, and use `block`/`rescue` rollback with a known-good backup. A failed validation must leave the active configuration unchanged. |
| P0-05 | Security | A surviving Ansible SSH session does not prove that a new SSH listener accepts a fresh connection. Reload success alone cannot prevent lockout. | After reload, run a controller-side port wait and establish a second SSH connection using an expected managed user/key. Reset the Ansible connection before removing the old port. Reboot and reconnect as the final proof. |
| P0-06 | Delivery | Collection and controller dependencies are not pinned even though the roles use modules outside `ansible-core`, including `ansible.posix` and `community.general`. Behavior can vary by workstation. | Add a versioned `collections/requirements.yml`, document the supported `ansible-core` version, and run from a reproducible virtual environment or execution environment. CI must install exactly those dependencies. |
...
- Distribution, major version, and CPU architecture are validated before mutation.
...
|---|---|---|
| P0 | A full `dist` upgrade and package autoremove/purge are enabled as normal convergence behavior. This can introduce broad change or remove packages during an ordinary application deployment. | Separate bootstrap, security patching, and full maintenance into explicit modes/tags. Require maintenance-window approval for broad upgrades, capture the package plan, and perform post-upgrade health/reboot checks. |
| P1 | Support is asserted by major version, but there is no tested image matrix. The OS role claims Debian support while the security layer is currently Ubuntu/UFW-oriented. | Publish an end-to-end supported-platform matrix. Test every declared distribution/version/architecture or narrow the declaration to Ubuntu until Debian-specific security behavior exists. |
| P1 | Hostname management does not establish cloud-init ownership, `/etc/hosts` behavior, or DNS/FQDN consistency. Cloud-init may rewrite the hostname. | Define short hostname/FQDN policy, coordinate `preserve_hostname`, validate forward/reverse resolution where required, and test after reboot. |
| P1 | Chrony is installed and checked, but the role does not own trusted time sources, NTS/authentication policy, maximum acceptable offset, or conflicts with another time service. | Select one time provider, manage sources, disable conflicting providers, and assert synchronization source plus maximum offset. Use NTS where the environment supports it. |
| P1 | Persisting `LC_ALL` globally can override application-specific locale behavior. | Prefer `LANG` as the global default and set individual `LC_*` values only when required. Validate with `localectl` and a fresh login session. |
| P1 | Mount validation checks that the mount path exists, not that source, filesystem type, options, and persistence match the declaration. | Assert `findmnt` source/type/options and `/etc/fstab`; add disk-space, inode, and swap policy checks. Test reboot persistence. |
| P1 | A required reboot is reported but not safely orchestrated. | Add an opt-in reboot phase with prechecks, timeout, reconnect, post-reboot kernel/service validation, and workload health gates. Never reboot merely because a file exists without an explicit policy. |
| P1 | Package repository trust, package holds, proxy/mirror settings, APT lock contention, and `needrestart` behavior are not defined. | Manage repository signing/keyrings and approved origins; add retries for transient APT locks; record holds; coordinate service restarts and reboot-required reporting. |
| P1 | Disabled/essential service validation needs to tolerate services that are intentionally absent while still detecting wrong active/enabled state. | Normalize absent, disabled, masked, enabled, and active states and test each case. |
...
| P1 | `developer`, `operator`, and `observer` are labels until later roles bind them to concrete resources. | Define authorization contracts consumed by application/runtime/service roles. Test least-privilege access and negative cases for every group. |
...
| Priority | Finding | Recommended feature and acceptance test |
|---|---|---|
| P0 | SSH port migration and post-reload connection validation are incomplete. | Implement P0-03 through P0-05 as one transactional migration workflow. Exercise failure injection and prove automatic rollback. |
...
- Pin `ansible-core`, `ansible.posix`, and `community.general`; document the controller Python version and supported target Python interpreters.
